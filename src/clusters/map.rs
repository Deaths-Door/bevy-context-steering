...
/// Global resource mapping unique `ClusterId` keys directly to their
/// corresponding root `Entity` handles.y
...
pub(crate) fn on_insert_cluster(
    ...
    if let Ok(Cluster(id)) = query.get(trigger.entity)
        && let Some(previous_entity) = clusters.0.insert(*id, trigger.entity)
    {
        warn!(
